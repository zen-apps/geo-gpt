...
def setup_environment():
    ...
    if provider == "openai":
        if not os.getenv("LLM_MODEL_OPENAI"):
            os.environ["LLM_MODEL_OPENAI"] = "gpt-4o-mini"
            logger.info("Setting default OpenAI model: gpt-4o-mini")
        
        if not os.getenv("OPENAI_API_KEY"):
            logger.warning("OPENAI_API_KEY not set - geocoding with LLM will fail")
            logger.warning("Set your OpenAI API key with: export OPENAI_API_KEY=your-key-here")
    
    elif provider == "google":
        if not os.getenv("LLM_MODEL_GOOGLE"):
            os.environ["LLM_MODEL_GOOGLE"] = "gemini-1.5-pro"
            logger.info("Setting default Google model: gemini-1.5-pro")
        
        if not os.getenv("GOOGLE_API_KEY"):
            logger.warning("GOOGLE_API_KEY not set - geocoding with LLM will fail")
            logger.warning("Set your Google API key with: export GOOGLE_API_KEY=your-key-here")
    
    elif provider == "anthropic":
        if not os.getenv("LLM_MODEL_ANTHROPIC"):
            os.environ["LLM_MODEL_ANTHROPIC"] = "claude-3-5-sonnet-latest"
            logger.info("Setting default Anthropic model: claude-3-5-sonnet-latest")
        
        if not os.getenv("ANTHROPIC_API_KEY"):
            logger.warning("ANTHROPIC_API_KEY not set - geocoding with LLM will fail")
            logger.warning("Set your Anthropic API key with: export ANTHROPIC_API_KEY=your-key-here")
        ...
        if not os.getenv("DEEPSEEK_API_KEY"):
            ...
            logger.warning("Set your DeepSeek API key with: export DEEPSEEK_API_KEY=your-key-here")
    
    logger.info(f"Environment setup complete. Using provider: {os.getenv('LLM_PROVIDER')}")
...
def main():
    ...
    print(f"Distance between Beverly Hills (90210) and New York (10001): {distance:.2f} km")
